...
<template>
    <ClientOnly>
        <div class="text-xl text-muted-foreground md:text-2xl">
            <div class="absolute w-0 h-0 overflow-hidden" role="marquee"
                aria-label="a description with typewriter effect">
                {{ description }}
            </div>
            <div class="inline overflow-hidden px-1" :class="{ 'animate-background-blink': isIdle }" aria-hidden="true">
                {{ content }}
            </div>
            <div class="inline-block w-2 text-2xl md:text-3xl font-bold text-blue-400 rounded-sm select-none "
...
export default defineComponent({
    ...
    setup(props) {
        const content = ref(props.snippets[0])
        const description = ref(props.snippets[0])
        ...
        function nextIdx() {
            return (curIdx + 1) % props.snippets.length
        }

        function runTypewriter() {
            if (!transition) {
                const from = props.snippets[curIdx]
                curIdx = nextIdx()
                const to = props.snippets[curIdx]
