...
// Dev Environment
const devEnvMacos = [
  { icon: Terminal, label: 'Warp Terminal', color: 'bg-gray-700', iconColor: 'text-green-400' },
  { icon: Code2, label: 'VS Code + Xcode', color: 'bg-blue-600/20', iconColor: 'text-blue-400' },
  { icon: Coffee, label: 'Homebrew', color: 'bg-orange-600/20', iconColor: 'text-orange-400' }
]

const devEnvWindows = [
  { icon: Code2, label: 'Visual Studio', color: 'bg-purple-600/20', iconColor: 'text-purple-400' },
  { icon: Terminal, label: 'WSL 2 (Ubuntu)', color: 'bg-yellow-600/20', iconColor: 'text-yellow-400' },
...
<template>
  <section class="min-h-screen pt-32 pb-20 px-4 max-w-7xl mx-auto animate-fade-in">

    <!-- Page Header -->
    <div class="mb-12">
      <h1 class="text-5xl md:text-7xl font-bold text-on-background mb-4 tracking-tight">
        {{ t('about.title') }} <span class="text-primary">{{ t('about.me') }}</span>
      </h1>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6 auto-rows-[minmax(180px,auto)]">

      <!-- 1. Main Profile Card - Large -->
      <div class="col-span-1 md:col-span-2 lg:col-span-2 row-span-2 bg-surface-container-high rounded-[2.5rem] p-8 flex flex-col justify-between relative overflow-hidden group border border-outline-variant/10">
        <div class="absolute top-0 right-0 w-64 h-64 bg-primary/10 rounded-full blur-3xl -mr-16 -mt-16 transition-all duration-700 group-hover:bg-primary/20"></div>

        <div class="relative z-10">
          <div class="w-20 h-20 rounded-full overflow-hidden border-4 border-surface mb-6 shadow-xl">
          ...
          <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-secondary-container text-on-secondary-container text-sm font-medium mb-6">
        ...
        <div class="mt-8 pt-6 border-t border-outline-variant/20 flex gap-4">
          <div class="flex flex-col">
            <span class="text-3xl font-bold text-primary">{{ stats.yearsExp }}</span>
            <span class="text-sm text-on-surface-variant">{{ t('about.yearsExp') }}</span>
          </div>
          <div class="flex flex-col border-l border-outline-variant/20 pl-4">
            <span class="text-3xl font-bold text-primary">{{ stats.projects }}</span>
            <span class="text-sm text-on-surface-variant">{{ t('about.projects') }}</span>
          </div>
          <div class="flex flex-col border-l border-outline-variant/20 pl-4">
            <span class="text-3xl font-bold text-primary">{{ stats.coffeeFueled }}</span>
            <span class="text-sm text-on-surface-variant">{{ t('about.coffee') }}</span>
          </div>
        </div>
      </div>

      <!-- 2. Location Widget - Map Style -->
      <div class="col-span-1 md:col-span-1 row-span-1 bg-surface-container rounded-[2.5rem] relative overflow-hidden group border border-outline-variant/10 min-h-[200px]">
        <img
        ...
        <div class="absolute bottom-6 left-6 right-6">
          <div class="flex items-center gap-2 text-primary-container bg-primary/90 backdrop-blur-md px-4 py-2 rounded-xl w-fit mb-1">
            <MapPin :size="18" />
            <span class="font-bold">{{ t('about.location') }}</span>
          </div>
          <p class="text-xs text-white/70 pl-1">GMT+8 • {{ t('about.remote') }}</p>
        </div>
      </div>
      ...
      <div class="col-span-1 md:col-span-1 row-span-2 flex flex-col gap-4">
        <template v-for="social in socialLinks" :key="social.name">
          ...
          >
            <component :is="social.icon" :size="40" class="mb-2 group-hover:scale-110 transition-transform" />
          ...
          >
            <component :is="social.icon" :size="40" :class="[social.color, 'mb-2 group-hover:scale-110 transition-transform']" />
            <span class="font-medium text-on-surface">{{ social.name }}</span>
            <span class="text-xs text-on-surface-variant">{{ social.handle }}</span>
          </a>
        </template>
      </div>

      <!-- 4. Tech Stack -->
      <div class="col-span-1 md:col-span-2 lg:col-span-1 row-span-1 bg-surface-container rounded-[2.5rem] p-6 border border-outline-variant/10">
        <div class="flex items-center gap-2 mb-4 text-on-surface-variant">
          <Cpu :size="20" />
          <span class="font-bold uppercase tracking-wider text-xs">{{ t('about.techStack') }}</span>
        </div>
      ...
      <div class="col-span-1 md:col-span-2 lg:col-span-2 row-span-1 bg-[#1b2838] rounded-[2.5rem] p-6 relative overflow-hidden group border border-outline-variant/10">
        <div class="absolute inset-0 opacity-30 bg-[radial-gradient(circle_at_top_right,_var(--tw-gradient-stops))] from-blue-500 via-transparent to-transparent"></div>
        <div class="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4 h-full">
          <div>
            <div class="flex items-center gap-2 text-blue-300 mb-2">
              <Gamepad2 :size="20" />
              <span class="font-bold uppercase tracking-wider text-xs">{{ t('about.playing') }}</span>
            </div>
            <template v-if="steamStatus.currentGame">
              <h3 class="text-2xl font-bold text-white mb-1">{{ steamStatus.currentGame.name }}</h3>
          ...
          <div class="flex items-center gap-4">
            <div class="text-right hidden md:block">
              <div class="text-xs text-blue-200/60 font-mono">{{ t('about.steamStatus') }}</div>
              <div :class="[steamStatus.isOnline ? 'text-green-400' : 'text-gray-400', 'font-bold flex items-center justify-end gap-2']">
                <span v-if="steamStatus.isOnline" class="w-2 h-2 rounded-full bg-green-500 animate-pulse"></span>
      ...
      <div class="col-span-1 row-span-1 bg-tertiary-container text-on-tertiary-container rounded-[2.5rem] p-6 flex flex-col justify-between border border-white/5 relative overflow-hidden">
        ...
        <div class="flex items-center justify-between mb-4 relative z-10">
          ...
          <div v-if="nowPlaying.isPlaying" class="flex gap-1">
            ...
            <div class="w-1 h-4 bg-white/60 rounded-full animate-[bounce_1.2s_infinite]"></div>
            <div class="w-1 h-2 bg-white/40 rounded-full animate-[bounce_0.8s_infinite]"></div>
          </div>
        </div>
        <div class="relative z-10">
          <div class="text-xs opacity-70 uppercase tracking-widest mb-1">{{ t('about.onRepeat') }}</div>
      ...
      <!-- 7. Photography -->
      <div class="col-span-1 row-span-1 bg-surface-container-high rounded-[2.5rem] p-6 flex flex-col items-center justify-center text-center border border-outline-variant/10 group cursor-pointer hover:bg-surface-container transition-colors">
        <div class="w-12 h-12 rounded-2xl bg-secondary/20 text-secondary flex items-center justify-center mb-3 group-hover:scale-110 transition-transform">
          <Camera :size="24" />
        </div>
        <div class="font-bold text-on-surface">{{ t('about.photography') }}</div>
        <div class="text-xs text-on-surface-variant">{{ t('about.checkGallery') }}</div>
      </div>

      <!-- 8. Development Environment (Software) -->
      <div class="col-span-1 md:col-span-2 lg:col-span-2 row-span-1 bg-surface-container rounded-[2.5rem] overflow-hidden border border-outline-variant/10 flex flex-col md:flex-row">
        <!-- macOS Side -->
        ...
        <!-- Windows Side -->
        <div class="flex-1 p-8 bg-[#0f172a]/80 relative border-t md:border-t-0 md:border-l border-white/5">
    ...
    <div class="mt-12 text-center p-8 border-t border-outline-variant/10">
      <Heart :size="24" class="mx-auto text-error mb-4 animate-pulse" fill="currentColor" />
