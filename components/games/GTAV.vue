    ...
    crew: [
      {
        ...
        role: 'The Brains',
        side: 'left',
        img: 'https://ui-avatars.com/api/?name=Lester&background=1e3a8a&color=fff',
      },
      {
        name: 'Jimmy',
        role: 'The Son',
        side: 'right',
        img: 'https://ui-avatars.com/api/?name=Jimmy&background=1e3a8a&color=fff',
      },
    ],
  },
  {
    ...
    crew: [
      {
        ...
        role: 'Homie',
        side: 'left',
        img: 'https://ui-avatars.com/api/?name=Lamar&background=064e3b&color=fff',
      },
      {
        name: 'Chop',
        role: 'The Dog',
        side: 'right',
        img: 'https://ui-avatars.com/api/?name=Chop&background=064e3b&color=fff',
      },
    ],
  },
  {
    ...
    crew: [
      {
        ...
        role: 'CEO',
        side: 'left',
        img: 'https://ui-avatars.com/api/?name=Ron&background=7c2d12&color=fff',
      },
      {
        name: 'Wade',
        role: 'Cousin',
        side: 'right',
        img: 'https://ui-avatars.com/api/?name=Wade&background=7c2d12&color=fff',
      },
    ],
  },
  {
    ...
    crew: [
      {
        ...
        role: 'Handler',
        side: 'left',
        img: 'https://ui-avatars.com/api/?name=Agent14&background=ca8a04&color=fff',
      },
      {
        name: 'Pavel',
        role: 'Kapitan',
        side: 'right',
        img: 'https://ui-avatars.com/api/?name=Pavel&background=ca8a04&color=fff',
      },
    ],
  },
]
...
<template>
  ...
  >
      ...
      >
        ...
        <div
          ...
          :style="{
            ...
            transform: `scale(1.1) translate(${mousePos.x * -1}px, ${
              mousePos.y * -1
            }px)`,
          }"
        />
    ...
    >
      ...
      <div
        class="relative w-full max-w-7xl h-full flex items-end justify-center pb-0 md:pb-10"
      >
        <!-- LEFT CREW MEMBER -->
        <div
          ...
          :style="{
            transform: `translateX(${mousePos.x * -2}px) scale(${
              activeCharIndex === activeChar.id ? 1 : 0.9
            })`,
            opacity: 1,
          }"
        >
          <div class="relative group cursor-pointer">
            ...
            />
            <div
              class="absolute -top-12 left-0 opacity-0 group-hover:opacity-100 transition-opacity bg-black/80 text-white p-2 rounded border-l-4 border-[var(--char-color)]"
            >
              <div class="font-bold uppercase text-sm">
                {{ getCrewMember('left')?.name }}
              </div>
              <div class="text-xs text-[var(--char-color)]">
                {{ getCrewMember('left')?.role }}
              </div>
            </div>
          </div>
        </div>
        ...
        >
          <div class="relative group cursor-pointer">
            ...
            />
            <div
              class="absolute -top-12 right-0 text-right opacity-0 group-hover:opacity-100 transition-opacity bg-black/80 text-white p-2 rounded border-r-4 border-[var(--char-color)]"
            >
              <div class="font-bold uppercase text-sm">
                {{ getCrewMember('right')?.name }}
              </div>
              <div class="text-xs text-[var(--char-color)]">
                {{ getCrewMember('right')?.role }}
              </div>
            </div>
          </div>
        </div>
        ...
        <div
          ...
          :style="{
            transform: `translate(${mousePos.x * -4}px, ${mousePos.y * 1}px)`,
          }"
        >
          <img
            :key="activeChar.id"
    ...
    <!-- ================= UI OVERLAY (HUD) ================= -->
    <div
      class="absolute inset-0 z-30 pointer-events-none flex flex-col justify-between p-6 md:p-12 pt-28 md:pt-32"
    >
      ...
      <div class="flex justify-between items-start pointer-events-auto">
        ...
        <div class="text-right text-white drop-shadow-md">
          ...
          <div
            class="text-sm font-bold uppercase tracking-wide opacity-80 flex justify-end gap-2 items-center"
          >
      ...
      <div class="flex justify-between items-end pointer-events-auto">
        <!-- Left: Stats Bars -->
        <div
          class="hidden md:block bg-black/60 p-4 rounded-lg backdrop-blur-sm w-64 space-y-2 border-l-2 border-white/20"
        >
          <h3
            class="text-white font-bold uppercase text-sm mb-2 border-b border-white/20 pb-1 flex justify-between"
          >
            Skills
            <span :style="{ color: activeChar.color }">{{
              activeChar.role
            }}</span>
          </h3>
          <div
          ...
          >
            <div
              class="w-20 text-[10px] font-bold text-white/70 uppercase text-right"
            >
              {{ stat }}
            </div>
        ...
        <div
          class="relative w-48 h-48 md:w-64 md:h-64 group rounded-full overflow-hidden shadow-[0_0_50px_rgba(0,0,0,0.5)]"
        >
          <!-- Decorative Ring -->
          <div
            class="absolute inset-0 rounded-full border-[12px] border-black/80 backdrop-blur-sm pointer-events-none z-20"
          />
            ...
            <div
              :class="[
                ...
                activeCharIndex === 1
                  ...
                  : 'border-gray-500 grayscale opacity-60',
              ]"
            >
              <img
                :src="CHARACTERS[1].hero"
          ...
          <!-- Center Hub -->
          <div
            class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-1/4 h-1/4 bg-black/90 rounded-full border-4 border-gray-700 z-20 flex items-center justify-center pointer-events-none shadow-lg"
          >
            <span class="text-[10px] text-white/50 font-bold uppercase"
              >Switch</span
            >
          </div>
        </div>
